...
myFile :: String               -- Sets file manager
myFile = "thunar"

myTorrent :: String               -- Sets torrrent engine
myTorrent = "transmission-gtk"

myBlue :: String                 --Sets bluetooth 
myBlue = "blueman-manager"

mySearch :: String               -- Sets search engine
mySearch = "catfish"

myReader :: String               -- Sets file reader
myReader = "okular"

myVideo :: String               -- Sets audio-video
myVideo = "vlc"

myEditor :: String
myEditor = "pluma"  -- Sets pluma as editor
...
myTerminal1 :: String
myTerminal1 = "xterm"    -- Sets alternate terminal

myBrowser :: String
myBrowser = "google-chrome-stable "  -- Sets chrome as browser

myEmail :: String
myEmail = "thunderbird "  -- Sets thunderbird as e-mail client

myBrowser1 :: String
myBrowser1 = "opera "  -- Sets opera as alternate browser
...
--  spawn "killall conky"   -- kill current conky on each restart
  ...
  spawnOnce "lxsession"
--  spawnOnce "picom"
  spawnOnce "picom -b --backend glx" --this enables Gausiian Blurring in picom
  spawnOnce "nm-applet"
-- spawnOnce "volumeicon"
  spawnOnce "dunst"
  spawnOnce "xscreensaver --nosplash"
  spawnOnce "google-chrome-stable"
  spawnOnce "libinput-gestures-setup stop desktop autostart start" --starts swipe gestures see https://github.com/bulletmark/libinput-gestures to install it. 
  spawn "/usr/bin/emacs --daemon" -- emacs daemon for the emacsclient
--  spawnOnce "~/.local/bin/updates_notify.sh" -- check for updates
...
--  spawnOnce "xargs xwallpaper --stretch < ~/.cache/wall"
  ...
  spawnOnce "pactl -- set-sink-volume 0 30%" -- set initial volume of speakers
  setWMName "LG3D"
...
myWorkspaces = [" chrome ", " kitty ", " thunar ", " search ", " torrent ", " blueman ", " vlc ", " ranger ", " email ", " tmux "]
...
myManageHook = composeAll
 ...
 -- , className =? "vlc"         --> doCenterFloat
  ...
  , className =? "opera"            --> doShift ( myWorkspaces !! 8 )
...
myKeys c =
  ...
  , ("M1-<Return>", addName "Launch terminal" $ spawn (myTerminal1))
...
--, ("<XF86AudioMute>", addName "Toggle audio mute"   $ spawn "amixer set Master toggle | ~/.local/bin/mute.sh")
...
--, ("<XF86AudioLowerVolume>", addName "Lower vol"    $ spawn "pactl set-sink-volume @DEFAULT_SINK@ -5% | ~/.local/bin/changeVolume.sh")
--, ("<XF86AudioRaiseVolume>", addName "Raise vol"    $ spawn "pactl set-sink-volume @DEFAULT_SINK@ +5% | ~/.local/bin/changeVolume.sh")
  
  , ("<XF86AudioLowerVolume>", addName "Lower vol"    $ spawn "pactl set-sink-volume @DEFAULT_SINK@ -5% | ~/.local/bin/vol.sh down")
  , ("<XF86AudioRaiseVolume>", addName "Raise vol"    $ spawn "pactl set-sink-volume @DEFAULT_SINK@ +5% | ~/.local/bin/vol.sh up")
  , ("<XF86AudioMute>", addName "Toggle audio mute"   $ spawn "amixer set Master mute | ~/.local/bin/vol.sh mute")
  
  
--  , ("<XF86MonBrightnessUp>",  addName "Raise bright"                       $ spawn "light -A 5 | ~/.local/bin/changebrightness.sh")
--  , ("<XF86MonBrightnessDown>", addName "Lower Bright"                       $ spawn "light -U 5 | ~/.local/bin/changebrightness.sh")

--  , ("<XF86MonBrightnessUp>",  addName "Raise bright"                       $ spawn "brightnessctl set +10%")
--  , ("<XF86MonBrightnessDown>", addName "Lower Bright"                       $ spawn "brightnessctl set 10%-")

  , ("<XF86MonBrightnessUp>",  addName "Raise bright"                       $ spawn "~/.local/bin/brightup.sh")
  , ("<XF86MonBrightnessDown>", addName "Lower Bright"                       $ spawn "~/.local/bin/brightdown.sh")
...
-- My Keybindings
  ...
  [  ("M-M1-1", addName "Google"     $spawn (myBrowser))
    ...
    , ("M1-t", addName "thunderbird"       $spawn (myEmail)) 
    , ("M-d", addName "rofi"         $spawn ("rofi -show drun -dpi 1"))
    , ("M-M1-d", addName "rofi1"         $spawn ("rofi -show combi run,window"))
    , ("M-b", addName "rofi-bluetooth"     $spawn ("~/.local/bin/rofi-bluetooth"))
    , ("M1-b", addName "broot"     $spawn ("kitty -e broot"))
    , ("M-M1-y", addName "youtube-viewer"     $spawn ("kitty -e ~/.local/bin/youtube-viewer.sh"))
    , ("M1-y", addName "pipe-viewer"     $spawn ("kitty -e ~/.local/bin/pipe-viewer"))
    , ("M1-v", addName "gtk-pipe--viewer"     $spawn ("~/.local/bin/gtk-pipe-viewer"))
